...
import { Constellation } from "./快捷函数/CreateMap-new";
import '../scritps/类/泛用类'
import { defineComponent } from "vue";
import { Pos2D } from "../scritps/类/泛用类";
const size = 128;
...
function drawmap(size:number,seed:number){
    ...
    const coordinate = new Pos2D(153,36)
    const smoothedMap1 = Constellation(coordinate,size)
    //const Blocks =BlockDivision(smoothedMap1)
